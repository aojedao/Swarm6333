  ...
  aruco_opencv_msgs__msg__ArucoDetection__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    aruco_opencv_msgs__msg__ArucoDetection * data =
      (aruco_opencv_msgs__msg__ArucoDetection *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!aruco_opencv_msgs__msg__ArucoDetection__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          aruco_opencv_msgs__msg__ArucoDetection__fini(&data[i]);
        }
        free(data);
        return false;
      }
